...
def euler_bungee(h, t_max=20, detectar_primer_maximo=True):
    """
    Método de Euler para resolver el problema de bungee jumping.
    ...
    while t < t_max:
        ...
        if detectar_primer_maximo and v_anterior > 0 and v_new < 0:
            y_max = max(y, y_new)
            break
...
if __name__ == "__main__":
    ...
    while h_actual >= 1e-6:
        _, _, _, y_max_num = euler_bungee(h_actual)
    ...
    if 'h_superior' in locals():
        ...
        for h in h_refinado:
            _, _, _, y_max_num = euler_bungee(h)
            error = abs(y_max_num - y_max_analitico) / y_max_analitico * 100
